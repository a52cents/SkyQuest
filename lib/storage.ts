...
export function getObservations(): Observation[] {
  ...
  const observations = stored.filter((item): item is Observation => {
    ...
    return typeof value.id === "string" && typeof value.createdAt === "string" &&
      typeof value.questTitle === "string" && typeof value.target === "string" &&
      (value.status === "seen" || value.status === "missed") && typeof value.visibilityScore === "number";
  });
...
function isTargetType(value: unknown): value is QuestTargetType {
  return ["moon", "planet", "star", "asterism", "constellation", "star_cluster", "galaxy", "meteor_shower", "satellite", "free_observation"].includes(String(value));
}

function isAchievementId(value: unknown): value is AchievementId {
  return ["first-look", "planet-tour", "night-landmarks", "orbital-watcher", "persistent", "explorer", "confirmed-watcher"].includes(String(value));
}
...
export function addObservation(
  ...
  now = new Date(),
): { observation: Observation; profile: ProgressProfile; reward: ProgressReward; persisted: boolean } {
