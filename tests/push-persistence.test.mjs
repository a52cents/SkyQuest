...
test("Supabase migration enforces unique endpoints, RLS, and an atomic daily claim", () => {
  ...
  assert.match(migrationSource, /interval '24 hours'/i);
...
test("test pushes use the same atomic 24-hour limit as scheduled pushes", () => {
  assert.match(testRouteSource, /await claimDailyPushSlot\(subscription\.endpoint\)/);
