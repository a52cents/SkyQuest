...
export function PageShell({
  ...
  return (
    <main className={joinClasses("mx-auto flex min-h-[100dvh] w-full max-w-[600px] flex-col px-5 pb-6", className)}>
        ...
        <div className="flex items-center justify-between gap-4">
          <div className="flex min-w-0 items-center gap-3">
            <svg viewBox="0 0 24 24" className="h-6 w-6 shrink-0" aria-hidden="true">
              ...
              <path d="M12 5 L7 16 L19 14 Z" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth=".5" />
            </svg>
            <div className="min-w-0">
              {hasHeading ? (
                <>
                  {eyebrow ? <p className="premium-kicker truncate">{eyebrow}</p> : <p className="premium-kicker truncate">SkyQuest</p>}
                  {title ? <h1 className="mt-0.5 truncate font-[Georgia,'Times_New_Roman',serif] text-[1.55rem] font-normal tracking-[-0.03em] text-white">{title}</h1> : null}
                </>
              ) : (
