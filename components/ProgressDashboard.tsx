    ...
    <div className="grid gap-4">
      <AppCard className="overflow-hidden border-accent/20 bg-[radial-gradient(circle_at_100%_0%,rgba(124,92,255,0.12),transparent_48%),#161619]">
        ...
        <div className="mt-3 flex items-end justify-between gap-4">
          <div>
            <h2 className="font-[Georgia,'Times_New_Roman',serif] text-2xl font-normal tracking-[-0.03em] text-white">{rank.current.name}</h2>
            <p className="mt-1 text-sm text-muted">{profile.totalXp} Éclats d’étoile</p>
          </div>
          <span className="text-3xl text-accent" aria-hidden="true">✦</span>
        </div>
      ...
      <AppCard as="section">
        ...
        <motion.div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3" variants={containerVariants} initial="hidden" animate="show">
          {COLLECTION_CATEGORIES.map((category) => {
            const count = profile.discoveredTargets.filter((target) => target.targetType === category.type).length;
            return (
              <motion.div key={category.type} className={`rounded-[12px] border p-3 ${count > 0 ? "border-accent/20 bg-accent/[0.07]" : "border-white/[0.06] bg-white/[0.02]"}`} variants={itemVariants}>
      ...
      <AppCard as="section">
        ...
        <motion.div className="mt-4 grid gap-2" variants={containerVariants} initial="hidden" animate="show">
          {achievements.map((achievement) => (
            <motion.div key={achievement.id} className={`rounded-[12px] border p-3 ${achievement.unlocked ? "border-success/20 bg-success/[0.06]" : "border-white/[0.06] bg-white/[0.02]"}`} variants={itemVariants}>
